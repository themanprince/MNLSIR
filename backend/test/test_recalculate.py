...
def test_recalculate_running_balance_accuracy(db_session, stock_service):
    ...
    m1 = StockMovement(store_id = store.id, product_id = product.id, movement_type = MovementType.RECIEVE, quantity_delta=Decimal("100"), movement_date=date(2026, 5, 10), created_at=datetime(2026, 5, 10, 10, 0), document_line_id = dummy_document_line.id, recorded_by = staff.id)
    m2 = StockMovement(store_id = store.id, product_id = product.id, movement_type = MovementType.ISSUE, quantity_delta=Decimal("-30"), movement_date=date(2026, 5, 12), created_at=datetime(2026, 5, 12, 10, 0), document_line_id = dummy_document_line.id, recorded_by = staff.id)
...
def test_recalculate_handles_back_dated_inserts(db_session, stock_service):
    ...
    m1 = StockMovement(store_id = store.id, product_id = product.id, movement_type = MovementType.RECIEVE, quantity_delta=Decimal("50"), movement_date=date(2026, 5, 10), created_at=datetime(2026, 5, 10, 12, 0), document_line_id = dummy_document_line.id, recorded_by = staff.id)
    m3 = StockMovement(store_id = store.id, product_id = product.id, movement_type = MovementType.ISSUE, quantity_delta=Decimal("-20"), movement_date=date(2026, 5, 14), created_at=datetime(2026, 5, 14, 12, 0), document_line_id = dummy_document_line.id, recorded_by = staff.id)
    ...
    m2_backdated = StockMovement(store_id = store.id, product_id = product.id, movement_type = MovementType.RECIEVE, quantity_delta = Decimal("100"), movement_date=date(2026, 5, 12), created_at=datetime(2026, 5, 12, 12, 0), document_line_id = dummy_document_line.id, recorded_by = staff.id)
...
def test_recalculate_tie_breaker_sorting_same_day(db_session, stock_service):
    ...
    entry_first = StockMovement(store_id = store.id, product_id = product.id, movement_type = MovementType.RECIEVE, quantity_delta=Decimal("10"), movement_date=target_day, created_at=datetime(2026, 5, 15, 8, 0), document_line_id = dummy_document_line.id, recorded_by = staff.id)
    entry_second = StockMovement(store_id = store.id, product_id = product.id, movement_type = MovementType.RECIEVE, quantity_delta=Decimal("3"), movement_date=target_day, created_at=datetime(2026, 5, 15, 12, 0), document_line_id = dummy_document_line.id, recorded_by = staff.id)
...
def test_recalculate_scope_isolation_by_store(db_session, stock_service):
    ...
    store1_movement = StockMovement(store_id = store1.id, product_id = product.id, quantity_delta = Decimal("15"), movement_type=MovementType.RECIEVE, movement_date = day, created_at = datetime.now(), document_line_id = dummy_document_line.id, recorded_by = staff.id)
    store2_movement = StockMovement(store_id = store2.id, product_id = product.id, quantity_delta = Decimal("60"), movement_type=MovementType.RECIEVE, movement_date = day, created_at = datetime.now(), document_line_id = dummy_document_line.id, recorded_by = staff.id)
