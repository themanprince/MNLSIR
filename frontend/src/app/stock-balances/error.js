...
export default function Error({ error, reset }) {
  useEffect(() => {
    // Pipeline system error logs securely to your diagnostic telemetry panel
    console.error("Dashboard Core Execution Fault:", error);
  }, [error]);
      ...
      <div className="bg-white rounded-2xl border border-rose-100 shadow-2xl p-6 md:p-8 text-center space-y-5">
        ...
        <div className="space-y-1.5">
          ...
          <p className="text-xs font-medium text-slate-500 max-w-sm mx-auto leading-relaxed">
            The application shell failed to securely map data from the FastAPI infrastructure. This could be due to a brief database connection break or local network loss.
          </p>
        </div>
        ...
        <div className="bg-slate-50 border border-slate-200/60 rounded-xl p-3 text-left">
          <span className="text-[9px] font-black uppercase tracking-widest text-slate-400 block mb-0.5">Telemetry Exception</span>
          <code className="text-xs font-mono text-rose-600 block break-words">
            {error?.message || "Unknown system link interface rejection."}
          </code>
        </div>
