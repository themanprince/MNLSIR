...
def test_update_historical_stockmovement_recalculates_forward_and_creates_log_FOR_POSITIVE_QUANTITY(db_session, stock_service):
    ...
    assert log.changed_by == operator_name
...
def test_insert_and_link_historical_movement_does_not_break_anchor_math(db_session, inventory_service, stock_service):
    ...
    explanatory_stock_movement = stock_service.insert_and_link_historical_movement(store_id = store.id, product_id = yoghurt.id, associated_stocktake_id=stock_take.id, movement_type=MovementType.ISSUE, quantity_delta = missing_quantity_delta, movement_date = date(2026, 5, 1), operator_name = "Prince", remarks = "Somebody was hungry and drank two yoghurts")
...
def test_delayed_stocktake_submission_corrects_timeline(db_session, inventory_service, stock_service):
    ...
    stock_movement1 = StockMovement(store_id = store.id, product_id = yoghurt.id, movement_date = date(2026, 5, 10), movement_type = MovementType.RECIEVE, quantity_delta = quantity_delta1)
    db_session.add(stock_movement1)
    db_session.commit()

    quantity_delta2 = Decimal("50")
    stock_movement2 = StockMovement(store_id = store.id, product_id = yoghurt.id, movement_date = date(2026, 5, 18), movement_type = MovementType.RECIEVE, quantity_delta = quantity_delta2)
    ...
    inventory_service.submit_stocktake(store_id = store.id, product_id = yoghurt.id, stocktake_date = date(2026, 5, 15), target_quantity = stocktake_quantity, operator_name="Prince", remarks = "testing delayed submission of stocktake")
...
def test_backdated_stockmovement_among_multiple_stocktakes_clamped_by_anchors(db_session, inventory_service, stock_service):
    ...
    stock_take1 = inventory_service.submit_stocktake(store_id=store.id, product_id = yoghurt.id, stocktake_date = date(2026, 5, 1), target_quantity = stocktake_quantity1, operator_name = "Prince", remarks="First StockK Take")
...
def test_historical_update_in_store_1_does_not_leak_or_alter_store_2(db_session, stock_service):
    ...
    store1_stock_movement = StockMovement(store_id = store1.id, product_id = yoghurt.id, movement_type = MovementType.RECIEVE, quantity_delta = store1_quantity_delta, running_balance = store1_quantity_delta)
...
def test_submit_stock_take_with_zero_quantity(db_session, inventory_service, stock_service):
    ...
    stock_movement1 = StockMovement(store_id = store.id, product_id = yoghurt.id, movement_date = date(2026, 5, 1), quantity_delta = quantity_delta1, movement_type = MovementType.RECIEVE)
    quantity_delta2 = Decimal("-20")
    stock_movement2 = StockMovement(store_id = store.id, product_id = yoghurt.id, movement_date = date(2026, 5, 23), quantity_delta = quantity_delta2, movement_type = MovementType.ISSUE)
    db_session.add_all([stock_movement1, stock_movement2])
    db_session.commit()
...
def test_invalid_association_raises_error(db_session, stock_service):
    ...
    with pytest.raises(UpdateStockMovementError):
        stock_service.update_historical_stockmovement(movement_id = 999, new_quantity_delta = Decimal("45"), remarks = "Testing function with unexistent stock mmovement", operator_name = "Prince")
    
    stock_movement = StockMovement(store_id = store.id, product_id = yoghurt.id, quantity_delta = Decimal("100"), movement_type = MovementType.RECIEVE)
    db_session.add(stock_movement)
    db_session.commit()
    db_session.refresh(stock_movement)

    with pytest.raises(AssociateStockMovementError):
        stock_service.associate_stock_movement_to_stocktake(movement_id = stock_movement.id, stocktake_id = 999, operator_name = "Not Me")
